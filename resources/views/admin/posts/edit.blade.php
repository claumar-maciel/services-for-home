...
@section('content')
    <form class="w-100 mt-4" action="{{route('admin.posts.update', ['post' => $post->id])}}" method="POST" enctype="multipart/form-data"> 
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            <div class="form-group my-3">
                <label>Título</label>
                <input type="text" class="form-control" placeholder="descreva a postagem com um título curto" name="title" value="{{ $post->content }}">
            </div>
    
            <div class="form-group my-3">
                <label>Conteúdo</label>
                <textarea type="text" class="form-control" placeholder="coloque o conteúdo da postagem aqui!" name="content">{{ $post->content }}</textarea>
            </div>
